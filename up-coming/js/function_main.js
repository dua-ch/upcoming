...
processPrivateLogs = function(pLogs) {
    pLogs = $(pLogs);
    if (prv_seen > 0) {
        pLogs.each(function(i, c) {
            if ($(c).find(".hunter_private").length) {
                var mark_css = $(
                    '<div class="msg_mark"><span data-testid="msg-dblcheck" aria-label=" Read " data-icon="msg-dblcheck" class="_3l4_3"><svg viewBox="0 0 18 13" width="16" height="15" class=""><path fill="currentColor" d="M15.01 3.316l-.478-.372a.365.365 0 0 0-.51.063L8.666 9.879a.32.32 0 0 1-.484.033l-.358-.325a.319.319 0 0 0-.484.032l-.378.483a.418.418 0 0 0 .036.541l1.32 1.266c.143.14.361.125.484-.033l6.272-8.048a.366.366 0 0 0-.064-.512zm-4.1 0l-.478-.372a.365.365 0 0 0-.51.063L4.566 9.879a.32.32 0 0 1-.484.033L1.891 7.769a.366.366 0 0 0-.515.006l-.423.433a.364.364 0 0 0 .006.514l3.258 3.185c.143.14.361.125.484-.033l6.272-8.048a.365.365 0 0 0-.063-.51z"></path></svg></span></div>'
                );
                if (
                    window.lastPrivSeenInConvo &&
                    lastPrivSeenInConvo >= $(c).data("id")
                ) {
                    mark_css.addClass("seen_mark");
                    $(c).addClass("prseen");
                }
                $(c).find(".pdate").prepend(mark_css);
            }
        });
    }
    return pLogs;
};
setPrivateMenuActions = function(target_close, target_locked) {
    if (target_close !== null) {
        if (target_close) {
            $("#pmenu_close_prv").show();
        } else {
            $("#pmenu_close_prv").hide();
        }
    }
    if (target_locked !== null) {
        if (target_locked) {
            $("#pmenu_lock_prv").hide();
            $("#pmenu_unlock_prv").show();
        } else {
            $("#pmenu_lock_prv").show();
            $("#pmenu_unlock_prv").hide();
        }
    }
};
chatReload = function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            if ("check" in response) {
				...
				var lrid = response.lrid;
				var pt = response.pt;
                speed = response.spd;
                inOut = response.acd;
				if (lrid && prv_seen > 0) {
                    if (lrid != lastPrivSeenInConvo) {
                        $(".prlog")
                            .not(".prseen")
                            .each(function(i, c) {
                                if (lrid >= $(c).data("id")) {
                                    $(c).find(".msg_mark").addClass("seen_mark");
                                }
                            });
                    }
                    lastPrivSeenInConvo = lrid;
                }
                if (response.act != userAction) {
                    ...
                    if (response.curp == $("#get_private").attr("value")) {
                        if (response.close_prv == 1) {
                            $("#close_prv_for_user").show();
                        }
                        if (response.close_prv == 0) {
                            $("#close_prv_for_user").hide();
                        }
						if (pt == 1) {
							$("#private_input").hide();
							...
							$("#priv_ask_msg").hide();
							$("#priv_req_sent").hide();
						} else if (response.perm_locked_private == 1) {
							$("#private_input").hide();
							$("#privacy_message").show();
							$("#priv_ask_msg").hide();
							$("#priv_req_sent").hide();
                        ...
                        if (privReload == 1) {
                            ...
                            if (pLogs == "") {
								...
								pLogs = processPrivateLogs(pLogs);
                    ...
                    if (response.room_color !== "") {
                        $(".chat_head").css("background", response.room_color);
                        $(".chat_footer").css("background", response.room_color);
                        $(".modal_top").css("background-color", response.room_color);
                        $("#large_modal_content .modal_top").css("background-color", response.room_color);
                        $(".left_title").css("background", response.room_color);
                        $(".top_background").css("background-color", response.room_color);
                        $(".background_header").css("background", response.room_color);
                        $(".emo_head").css("background", response.room_color);
                        $(".menui").css("color", response.room_color);
                        $(".panel_bar").css("color", response.room_color);
                        $(".sub_menu_icon").css("color", response.room_color);
                        $(".page_menu_icon").css("background", response.room_color);
                        $(".theme_color").css("color", response.room_color);
                        $(".default_btn").css("background", response.room_color);
                    } else {
                        $(".chat_head").css("background", "");
                        $(".chat_footer").css("background", "");
                        $(".modal_top").css("background-color", "");
                        $("#large_modal_content .modal_top").css("background-color", "");
                        $(".left_title").css("background", "");
                        $(".top_background").css("background-color", "");
                        $(".background_header").css("background", "");
                        $(".emo_head").css("background", "");
                        $(".menui").css("color", "");
                        $(".panel_bar").css("color", "");
                        $(".sub_menu_icon").css("color", "");
                        $(".page_menu_icon").css("background", "");
                        $(".theme_color").css("color", "");
                        $(".default_btn").css("background", "");
                    }
                    if (response.room_bg !== "") {
                        $("body").css("background", "url(upload/upload/" + response.room_bg + ")");
                    } else {
                        $("body").css("background", "");
                    }
                    if ("rset" in response) {
...
isInnactive = function () {
    if (curActive > inOut && !boomAllow(95) && inOut > 0) {
        logOut();
    }
};
roomBlock = function () {
...
setAllowAddFriend = function (s) {
    var allow_friend = $(s).val();
    $.post("system/action_main.php", {
...
setPointsAccess = function (s) {
    var points_access = $(s).val();
    $.post("system/action_main.php", {
        points_access: points_access,
        token: utk
    }, function (response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
setFriendsAccess = function (s) {
    var friends_access = $(s).val();
    $.post("system/action_main.php", {
        friends_access: friends_access,
        token: utk
    }, function (response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
askUserToTalk = function () {
    $.post("system/action_main.php", {
        ask_user_to_talk: $("#get_private").attr("value"),
        token: utk
    }, function (response) {
        if (response == 1) {
            $("#priv_ask_msg").hide();
...
acceptPrvAsk = function (user, notify) {
    $.post("system/action_main.php", {
        accept_user_prv: user,
        accept_user_notify: notify,
        token: utk
    }, function (response) {
        hideModal();
    });
};
refusePrvAsk = function (user, notify) {
    $.post("system/action_main.php", {
        refuse_user_prv: user,
        refuse_user_notify: notify,
        token: utk
    }, function (response) {
        hideModal();
    });
};
closePrvThisUser = function () {
    $.post("system/action_main.php", {
        close_prv_this_user: $("#get_private").attr("value"),
        token: utk
    }, function (response) {
        if (response == 1) {
            callSaved(system.actionComplete, 1);
...
$(function () {
    $("#private_panel").draggable({handle: ".private_drag", containment: "document"});
});
closeList = function () {
    resetAvMenu();
    hidePanel();
...
emojiCount = function(s) {
    var regexp = RegExp('(?:[\u2700-\u27bf]|(?:\ud83c[\udde6-\uddff]){2}|[\ud800-\udbff][\udc00-\udfff])[\ufe0e\ufe0f]?(?:[\u0300-\u036f\ufe20-\ufe23\u20d0-\u20f0]|\ud83c[\udffb-\udfff])?(?:\u200d(?:[^\ud800-\udfff]|(?:\ud83c[\udde6-\uddff]){2}|[\ud800-\udbff][\udc00-\udfff])[\ufe0e\ufe0f]?(?:[\u0300-\u036f\ufe20-\ufe23\u20d0-\u20f0]|\ud83c[\udffb-\udfff])?)*', 'g');
    var match;
    var n = 0;
    while ((match = regexp.exec(s)) !== null) n++;
    var m = (s.split(':').length - 1) / 2;
    if (m >= 2) {
        n += m;
    }
    return n;
...
resetYoutube = function () {
	var cont = document.getElementById('container_stream');
    cont.style.removeProperty('left');
    $("#yt_tog").addClass("privhide");
    scrollPriv(1);
};
toggleYoutube = function (type) {
    if (type == 1) {
        $("#yt_tog").removeClass("privhide");
        $("#container_stream").css("left", "-10000000000px");
    }
    if (type == 2) {
        resetYoutube();
    }
...
previewImage = function(type) {
    ...
    if (file_data.type.indexOf('image/') === 0) {
        ...
        reader.onload = function(e) {
            var targetClick = type == 0 ? "uploadPrivate();" : 'uploadChat();';
            var body = '<div style="background: black; position: relative;height:' + (window.innerHeight * 0.9) + 'px;vertical-align: middle;"></div><div class="absolute_center"><img src="' + e.target.result + '" style="max-width: 100%; max-height: 100%;" class="absolute_center"><div style="position: absolute; bottom: 0; background: rgba(0, 0, 0, .5);width: 100%; height: 50px;"><button onclick="' + targetClick + '" class="reg_button ok_btn cancel_modal prev_right_btn csend"><i class="fa fa-paper-plane"></i> ارسال</button><button class="prev_left_btn cancel_modal reg_button delete_btn csend"><i class="fa fa-times"></i> الغاء</button></div></div></div>';
            showEmptyModal(body);
        }
...
saveRoomStyle = function () {
    $.post("system/action_main.php", {
        room_style_color: $("#room_style_color").val(),
        token: utk
    }, function (response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
saveRoom = function () {
    $.post("system/action_room.php", {
        ...
        allow_points_inroom: $("#allow_points_inroom").val(),
        this_room_access: $("#this_room_access").val(),
        ...
        if (myNews.length > 2000) {
            ...
            $.post("system/action_main.php", {
                ...
                if (response == 0) {
                    ...
                    callSaved("لقد تخطيت الحد المسموح لاستخدام الاخبار", 3);
                    waitNews = 0;
                    return false;
                } else {
...
sendUserPoints = function (target) {
    $.post("system/action_profile.php", {
        target: target,
        send_points: $("#send_user_points").val(),
        token: utk
    }, function (response) {
        if (response == 1) {
            callSaved("تم ارسال النقاط", 1);
        } else if (response == 2) {
            callSaved("لا تمتلك نقاط كافية", 3);
        } else {
            callSaved(system.error, 3);
        }
    });
...
clearThisRoom = function () {
    $.post("system/chat_command.php", {
        token: utk
    }, function (response) {
        if (response.code == 99) {
            noAction();
...
// clearEmoji = function(s) {
    // return s.replace(/(?:[\u2700-\u27bf]|(?:\ud83c[\udde6-\uddff]){2}|[\ud800-\udbff][\udc00-\udfff])[\ufe0e\ufe0f]?(?:[\u0300-\u036f\ufe20-\ufe23\u20d0-\u20f0]|\ud83c[\udffb-\udfff])?(?:\u200d(?:[^\ud800-\udfff]|(?:\ud83c[\udde6-\uddff]){2}|[\ud800-\udbff][\udc00-\udfff])[\ufe0e\ufe0f]?(?:[\u0300-\u036f\ufe20-\ufe23\u20d0-\u20f0]|\ud83c[\udffb-\udfff])?)*/g, function(m) {
        // return '<span class="emoji">' + m + '</span>';
    // });
// }
processChatPost = function (message) {
	// var message = clearEmoji(message);
    $.post("system/chat_process.php", {
        content: message,
        snum: snum,
        token: utk
    }, function (response) {
        if (response == "") {} else if (response == 100) {
            checkRm(2);
        } else {
            $("#name").val("");
...
var waitexSaveNewGift = 0;
exSetNewRoomCover = function (item) {
    var file_data = $(item).prop("files")[0];
    if ($(item).val() === "") {
        callSaved(system.noFile, 3);
    } else {
        if (waitexSaveNewGift == 0) {
            waitexSaveNewGift = 1;
            var form_data = new FormData();
            form_data.append("file", file_data);
            form_data.append("token", utk);
            $.ajax({
                url: "system/action_room_cover.php",
                dataType: "json",
                cache: false,
                contentType: false,
                processData: false,
...
$(document).ready(function () {
    ...
    $("#container_show_chat").on("click", "#show_chat .username", function () {
		...
		window.usernameEmojiCount += emojiCount($(this).text());
    });
    ...
    $("#main_input").submit(function (event) {
        ...
        if (message == "") {
            ...
            event.preventDefault();
            chatInput();
        } else if (emojiCount(message) - window.usernameEmojiCount > 10) {
            warningBox('<div class="pad_box bold error">لا يمكنك ارسال اكثر من 10 اسمايلات</div>');
        } else {
            chatInput();
            if (waitReply == 0) {
                waitReply = 1;
            ...
            if (pWait == 0) {
                ...
                $.post("system/private_process.php", {
                    ...
                    if (response == 20) {
                        ...
                        $("#message_content").focus();
                        $("#private_content ul").append(processPrivateLogs(response));
                        scrollPriv(1);
                    } pWait = 0;
                });
            } else {
    ...
    $(function () {
        if ($(window).width() > 1024) {
            $("#private_box").draggable({handle: "#private_top", containment: "document"});
        }
    });
	...
	confirmClearPrivateList = function () {
        hideAll();
        $.post("system/box/privatelist_delete.php", {
            token: utk
        }, function (response) {
            overModal(response);
        });
	...
	$('#container_stream').on('click', '#hide_stream', function() {
        $('#wrap_stream').toggleClass('hide');
        $('#wrap_stream video').toggleClass('hide');
        $('#hide_stream').toggleClass('fa-plus');
    });

    $(function () {
        $("#container_stream").draggable({containment: "document", scroll: false});
    });
