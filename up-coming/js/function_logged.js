$(document).ready(function () {
	...
	$(document).on('click', '.moving_name', function () {
        var curColor = $(this).attr('data');
        if ($('.user_color').attr('data') == curColor) {
            $('.bccheck').remove();
            $('.user_color').attr('data', 'user');
			exSaveMovingColor('user');
        } else {
            $('.bccheck').remove();
            $(this).append('<i class="fa fa-check bccheck"></i>');
            $('.user_color').attr('data', curColor);
			exSaveMovingColor(curColor);
        }
        previewName();
    });
    ...
    $(document).on('click', '.ex_choice', function () {
        var curColor = $(this).attr('data');
        if ($('.user_color').attr('data') == curColor) {
            $('.bccheck').remove();
            $('.user_color').attr('data', 'user');
            ...
            $('.bccheck').remove();
            $(this).append('<i style="font-size: 11px;color: #fff;margin-top: 6px;" class="fa fa-check bccheck"></i>');
            $('.user_color').attr('data', curColor);
        }
    });
    $(document).on('click', '.ex_name_bg', function () {
        var curColor = $(this).attr('data');
        if ($('.user_color').attr('data') == curColor) {
            $('.bccheck').remove();
            $('.user_color').attr('data', 'user');
    ...
    $(document).on('click', '.ex_list_bg', function () {
        var curColor = $(this).attr('data');
        if ($('.user_color').attr('data') == curColor) {
            $('.bccheck').remove();
            $('.user_color').attr('data', 'user');
    ...
    $(document).on('click', '.ex_list_glow', function () {
        var curColor = $(this).attr('data');
        if ($('.user_color').attr('data') == curColor) {
            $('.bccheck').remove();
            $('.user_color').attr('data', 'user');
    ...
    $(document).on('click', '.pro_choice', function () {
        var curColor = $(this).attr('data');
        var id = $('.my_pro_color').val();
		var proClass, i;
		proClass = document.querySelectorAll(".example");
        if ($('.my_pro_color').attr('data') == curColor) {
            $('.bccheck').remove();
            $('.my_pro_color').attr('data', '');
			$('#editprofile_tabs_background').removeClass().addClass('modal_menu');
            $('#meditprofile').removeClass();
        } else {
            $('.bccheck').remove();
            $(this).append('<i class="bccheck fa fa-check"></i>');
            $('.my_pro_color').attr('data', curColor);
			$('#editprofile_tabs_background').removeClass().addClass('modal_menu').addClass(curColor);
            $('#meditprofile').removeClass().addClass(curColor);
        }
    });
    $(document).on('click', '.pro_shadow', function () {
        ...
        if ($('.my_pro_shadow').attr('data') == curColor) {
			...
			$('#editprofile_shadow').removeClass();
        } else {
            $('.bccheck').remove();
            $(this).append('<i class="bccheck fa fa-check"></i>');
            $('.my_pro_shadow').attr('data', curColor);
			$('#editprofile_shadow').removeClass();
            $('#editprofile_shadow').addClass(curColor);
        }
    });
    $(document).on('click', '.ex_av_border', function () {
        var curColor = $(this).attr('data');
        var id = $(this).val();
        if ($('.user_color').attr('data') == curColor) {
            $('.bccheck').remove();
...
var waitRoomBg = 0;
uploadRoomBg = function() {
    var file_data = $("#ex_room_bg").prop("files")[0];
    if ($("#ex_room_bg").val() === "") {
        callSaved('يجب عليك اختار خلفية أولا', 3);
    } else {
        if (waitRoomBg == 0) {
            waitRoomBg = 1;
            uploadIcon('r_bg_ico', 1);
            var form_data = new FormData();
            form_data.append("room_bg", file_data)
            form_data.append("token", utk)
            $.ajax({
                url: "system/action_main.php",
                dataType: 'json',
                cache: false,
                contentType: false,
                processData: false,
                ...
                success: function(response) {
                    if (response == 1) {
                        ...
                        callSaved('تم الرفع', 1);
                    } else {
                        callSaved(system.error, 3);
                    }
                    uploadIcon('r_bg_ico', 2);
                    waitRoomBg = 0;
                },
                error: function() {
                    callSaved(system.error, 3);
...
var waitRoomIcon = 0;
uploadRoomIcon = function() {
    var file_data = $("#ex_room_icon").prop("files")[0];
    if ($("#ex_room_icon").val() === "") {
        callSaved('يجب عليك اختيار خلفية أولا', 3);
    } else {
        if (waitRoomIcon == 0) {
            waitRoomIcon = 1;
            uploadIcon('r_icon_ico', 1);
            var form_data = new FormData();
            form_data.append("room_icon", file_data)
            form_data.append("token", utk)
            $.ajax({
                url: "system/action_main.php",
                dataType: 'json',
...
registerGuest = function () {
    ...
    if (! gpass.match(/\d/)) {
        callSaved('يجب ان تحتوي كلمة المرور على رقم', 3);
        $('#new_guest_password').val("");
        return false;
    } else if (! gpass.match(/[A-Z]/)) {
        callSaved('يجب ان تحتوي كلمة المرور على حرف انجليزي كابيتال', 3);
        $('#new_guest_password').val("");
        return false;
    } else if (! gpass.match(/[A-z]/)) {
        callSaved('يجب ان تحتوي كلمة المرور على حرف و اكثر', 3);
        $('#new_guest_password').val("");
        return false;
    } else if (gpass.length < 8) {
        callSaved('يجب ان تكون كلمة المرور اكثر من 8 احرف وارقام', 3);
        $('#new_guest_password').val("");
        return false;
    } else if (gname == '' || gpass == '' || gemail == '') {
...
lockPrivateThisUser = function() {
    var ign = $('#get_private').attr('value');
    lockPrivate(ign, function() {
        setPrivateMenuActions(null, true);
    });
};
unlockPrivateThisUser = function() {
    var ign = $('#get_private').attr('value');
    unlockPrivate(ign, function() {
        setPrivateMenuActions(null, false);
    });
};
closeUserPriv = function(id, cb) {
    $.post('system/action_main.php', {
        close_priv: id,
        token: utk,
    }, function(response) {
...
closeThisUserPrv = function() {
    var ign = $('#get_private').attr('value');
    closeUserPriv(ign, function() {
        setPrivateMenuActions(false, null);
    });
};
...
editProfileOptions = function () {
    $.post('system/box/edit_profile.php', {
        token: utk
    }, function (response) {
        showEmptyModal(response, 580);
        $('#personal_options_btn').click();
    });
}
getSpecialFrames = function () {
    $.post('system/box/photo_frames.php', {
        token: utk
    }, function (response) {
        overModal(response, 350);
    });
}
savePhotoFrame = function(frame, id) {
    $.post('system/action_main.php', {
...
delPhotoFrame = function(id) {
    $.post('system/action_main.php', {
        delete_photo_frame: id,
        token: utk,
    }, function(response) {
        callSaved(system.saved, 1);
        hideOver();
...
getPremiumOptions = function () {
    $.post('system/box/premium_options.php', {
        token: utk
    }, function (response) {
        overModal(response, 580);
    });
}
exSaveNewUserColors = function(target) {
    $.post('system/action_main.php', {
        user_target: target,
...
exSaveNewZ5rfa = function(target) {
    $.post('system/action_main.php', {
        user_target: target,
        user_name_z5rfa: $('#set_name_z5rfa').val(),
        token: utk,
    }, function(response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
exSaveNewNameBg = function(target) {
    $.post('system/action_main.php', {
        user_target: target,
        user_name_bg: $('.user_color').attr('data'),
        token: utk,
    }, function(response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
exSaveListBg = function(target) {
    $.post('system/action_main.php', {
        user_target: target,
        user_list_bg: $('.user_color').attr('data'),
        token: utk,
    }, function(response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
exSaveListGlow = function(target) {
    $.post('system/action_main.php', {
        user_target: target,
        user_list_glow: $('.user_color').attr('data'),
        token: utk,
    }, function(response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
exSaveAvBorder = function(target) {
    $.post('system/action_main.php', {
        user_target: target,
        user_av_border: $('.user_color').attr('data'),
        token: utk,
    }, function(response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
exDelAvBorder = function(target) {
    $.post('system/action_main.php', {
        del_user_av_border: target,
        token: utk,
    }, function(response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
saveProColor = function(id) {
    $.post('system/action_main.php', {
        my_newpro_color: $('.my_pro_color').attr('data'),
        my_pro_color_id: id,
        token: utk,
    }, function(response) {
        callSaved(system.saved, 1);
...
saveProShadow = function(id) {
    $.post('system/action_main.php', {
        my_newpro_shadow: $('.my_pro_shadow').attr('data'),
        my_pro_glow_id: id,
        token: utk,
    }, function(response) {
        callSaved(system.saved, 1);
...
addPointsToUser = function(id) {
    $.post('system/action_main.php', {
        points_for_user: $('#add_points_to_user').val(),
        user_pointer: id,
        token: utk,
    }, function(response) {
        if(response == 1){
...
takePointsToUser = function(id) {
    $.post('system/action_main.php', {
        take_points_to_user: $('#take_points_to_user').val(),
        user_pointer: id,
        token: utk,
    }, function(response) {
        if(response == 1){
...
delProColors = function(id) {
    $.post('system/action_main.php', {
        delete_profile_colors: id,
        token: utk
    }, function(response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
uploadProfileSong = function(id) {
    var file_data = $("#pro_song").prop("files")[0];
    if ($("#pro_song").val() === "") {
        callSaved('يجب عليك اختيار اغنيه ألا', 3);
    } else {
        if (waitAvatarSong == 0) {
            waitAvatarSong = 1;
...
var waitAvGif = 0;
exSaveAvGif = function(id) {
    var file_data = $("#ex_av_gif").prop("files")[0];
    if ($("#ex_av_gif").val() === "") {
        callSaved('يجب عليك اختيار اغنيه أولا', 3);
    } else {
        if (waitAvGif == 0) {
            waitAvGif = 1;
            uploadIcon('gift_av_ico', 1);
            var form_data = new FormData();
            form_data.append("gif_av", file_data)
            form_data.append("av_id", id)
            form_data.append("token", utk)
            $.ajax({
                url: "system/action_main.php",
                dataType: 'json',
...
delProfileSong = function(id) {
    $.post('system/action_main.php', {
        delete_profile_song: id,
        token: utk
    }, function(response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
delUserNameColors = function(id) {
    $.post('system/action_main.php', {
        delete_name_sp_color: id,
        token: utk
    }, function(response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
cancelThisSubscribe = function(id) {
    $.post('system/action_main.php', {
        cancel_user_esh: id,
        token: utk
    }, function(response) {
...
delAllUserWarns = function(id) {
    $.post('system/action_main.php', {
        del_all_user_warns: id,
        token: utk
    }, function(response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
giveRankEshtrak = function (target) {
    $.post('system/action_main.php', {
        eshtrak_user: target,
        eshtrak_rank: $('#eshtrak_rank').val(),
        eshtrak_time: $('#eshtrak_time').val(),
        token: utk
    }, function (response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
changePassword = function () {
    ...
    if (! newPass.match(/\d/)) {
        callSaved('يجب ان تحتوي كلمة المرور على رق', 3);
        $('#reg_password').val("");
        return false;
    } else if (! newPass.match(/[A-Z]/)) {
        callSaved('يجب ان تحتوي كلة المرور على حرف نجليزي كابيتال', 3);
        $('#reg_password').val("");
        return false;
    } else if (! newPass.match(/[A-z]/)) {
        callSaved('جب ان تحتوي كلمة لمرور على حرف او كثر', 3);
        $('#reg_password').val("");
        return false;
    } else if (newPass.length < 8) {
        callSaved('يجب ان تكون كلم المرور اكثر من 8 حرف وارقام', 3);
        $('#reg_password').val("");
        return false;
    } else {
        $.post('system/encoded/action_secure.php', {
...
changeMyUsername = function () {
    ...
    $.post('system/action_profile.php', {
        ...
        if (response == 1) {
            ...
            callSaved('تخطيت الحد المسموح لتغيير الاسم', 3);
            $('#my_new_username').val();
        } else {
            callSaved(system.error, 3);
            hideOver();
...
adminSaveName = function (u) {
    ...
    $.post('system/action_main.php', {
        ...
        if (response == 1) {
            ...
            callSaved('قد تخطيت الحد المسموح لتغيير اسماء الاعضاء', 3);
            $('#new_user_username').val();
        } else {
            callSaved(system.error, 3);
            hideOver();
...
exSaveMovingColor = function (color) {
    $.post('system/action_main.php', {
        my_moving_color: color,
        token: utk
    }, function (response) {
        if (response == 1) {
            callSaved(system.saved, 1);
...
adjustUserPoints = function (target, rank) {
    $.post('system/action_users.php', {
        adj_pts_user: target,
        adj_new_rank: rank,
        token: utk
    }, function () {});
}
logMoreMenu = function (target) {
    $.post('system/box/action_m.php', {
        id: target,
        token: utk
    }, function (response) {
        if (response == 0) {
...
changeRank = function (t, target) {
    ...
    $.post('system/action_main.php', {
        ...
        if (response == 0) {
            ...
            callSaved('لقد تخطيت احد المسموح لرفع الرتب', 3);
        } else {
            callSaved(system.error, 3);
        }
        hideOver();
...
systemHistory = function (id) {
    $.post('system/shistory.php', {
        sys_history: id,
        token: utk
    }, function (response) {
        if (response == 0) {
...
var curDelMsgs = 1000;
delUserMsgs = function (target) {
    var delTime = Math.round(new Date() / 1000);
    if (delTime > (curDelMsgs + 5)) {
        var delType = 0;
    } else {
        var delType = 1;
    }
...
getTrafficDetails = function() {
    $.post('system/box/traffic_list.php', {
        token: utk
    }, function(response) {
        showModal(response);
    });
...
getPointsTop = function () {
    $.post('system/box/points_top.php', {
        token: utk
    }, function (response) {
        showModal(response);
    });
}
getxBannedList = function () {
    $.post('system/box/xbanned_list.php', {
        token: utk
    }, function (response) {
        showModal(response);
    });
}
delFingerUser = function(id, user) {
    $.post('system/action_main.php', {
        token: utk,
        finger_target: id,
        user_finger: user,
    ...
    if (act == 'ban') {
        ...
        $.post('system/action.php', {
            ...
            if (response == 0) {
                ...
                callSaved(system.actionComplete, 1);
                processAction(act);
				if (act == 'warn' && $('#remove_msgs').is(':checked')) {
					delUserMsgs(target);
				}
            } else if (response == 2) {
                callSaved(system.alreadyAction, 3);
...
bannedUserDevice = function(u, device, dversion, dcode) {
	let castBox = new jBox("Confirm", {
		title:'<p style="direction:rtl; text-align: center;"><strong>حجب الجهاز :</strong></p>',
		content: 'هل أنت متأكد أنك تريد حجب هذا الجهاز من الدردشة',
		confirmButton: "حسنا",
		closeOnConfirm: true,
		closeOnEsc: false,
		closeOnClick: false,
			...
			$.post(
				...
				function(response) {
					if(response == 1){
						callSaved(system.actionComplete, 1);
						if(socket_on){
							socket_obj.needReload({type: "MINE", target: u});
						}
					} else {
						callSaved(system.error, 3);
					}
...
removeBannedDevice = function(device) {
	let castBox = new jBox("Confirm", {
		title: '<p style="direction:rtl; text-align: center;"><strong>فك حجب الجهاز :</strong></p>',
		content: 'هل أنت متأكد أنك تريد فك حجب هذا الجهاز ؟',
		confirmButton: "حسنا",
		closeOnConfirm: true,
		closeOnEsc: false,
		closeOnClick: false,
		...
		confirm: function() {
			$.post(
				"system/action_main.php", {
					remove_ban_device: device,
					token: utk,
				},
				function(response) {
					if(response == 1){
						callSaved(system.actionComplete, 1);
						$("#bandevice"+device).remove();
					} else {
						callSaved(system.error, 3);
					}
				}
...
bannedUserBrowser = function(b, u, v) {
	let castBox = new jBox("Confirm", {
		title: '<p style="direction:rtl; text-align: center;"><strong>حجب المتصفح :</strong></p>',
		content: 'هل انت متاكد انك تريد حجب هذا المتصفح ؟',
		confirmButton: "حسنا",
		closeOnConfirm: true,
		closeOnEsc: false,
		closeOnClick: false,
...
removeBannedBrowser = function(browser) {
	let castBox = new jBox("Confirm", {
		title: '<p style="direction:rtl; text-align: center;"><strong>فك حجب المتصفح :</strong></p>',
		content: 'هل انت متاكد انك تريد فك حجب هذا المتصفح ؟',
		confirmButton: "حسنا",
		closeOnConfirm: true,
		closeOnEsc: false,
		closeOnClick: false,
		...
		confirm: function() {
			$.post(
				"system/action_main.php", {
					remove_ban_browser: browser,
					token: utk,
				},
				function(response) {
					if(response == 1){
						callSaved(system.actionComplete, 1);
						$("#banbrowser"+browser).remove();
					} else {
						callSaved(system.error, 3);
					}
				}
...
bannedUserCountry = function(b, u) {
	let castBox = new jBox("Confirm", {
		title: '<p style="direction:rtl; text-align: center;"><strong>حجب الدولة :</strong></p>',
		content: 'هل انت متاكد انك تريد حجب هذه الدولة بالكامل ؟',
		confirmButton: "حسنا",
		closeOnConfirm: true,
		closeOnEsc: false,
		closeOnClick: false,
...
addCountryToBanned = function() {
	let castBox = new jBox("Confirm", {
		...
		confirm: function() {
			$.post(
				"system/action_main.php", {
					add_country_ban: $("#add_country_ban").val(),
					token: utk,
				},
				function(response) {
					if(response == 0){
						...
						callSaved(system.actionComplete, 1);
						$('#country_page_list').prepend(response);
						if($('.empty_zone').length){
							$('.empty_zone').remove();
						}
					}
				}
...
removeBannedCountry = function(country) {
	let castBox = new jBox("Confirm", {
		title: '<p style="direction:rtl; text-align: center;"><strong>فك حجب الدول :</strong></p>',
		content: 'هل أنت متاكد أنك تريد فك حجب هذه الدولة ؟',
		confirmButton: "حسنا",
		closeOnConfirm: true,
		closeOnEsc: false,
		closeOnClick: false,
		...
		confirm: function() {
			$.post(
				"system/action_main.php", {
					remove_ban_country: country,
					token: utk,
				},
				function(response) {
					if(response == 1){
						callSaved(system.actionComplete, 1);
						$("#bancountry"+country).remove();
					} else {
						callSaved(system.error, 3);
					}
				}
