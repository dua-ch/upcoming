...
function colorChoice($sel, $type, $min = 1)
{
    ...
    for ($n = $min; $n <= $cody['color_count']; $n++) {
        ...
        if (boomAllow(100)) {
            $show_white = '<div data="bcolor78" class="color_switch ' . $c . ' bcback78">' . $add_sel . '</div>';
        }
        $show_c .= '<div data="' . $val . '" class="color_switch ' . $c . ' ' . $back . '">' . $add_sel . '</div>';
    }
    return $show_white . $show_c;
}
...
function validName($name)
{
    ...
    $reserved = array('system__', 'public__', 'my_notice', 'user_rank', 'user_id', 'user_name');
    ...
    if (mb_strlen($name, 'UTF-8') <= $data['max_username'] && !ctype_digit($name) && mb_strlen($name, 'UTF-8') >= 2) {
        return true;
    }
    return false;
}
...
function removeOnair($target)
{
    ...
    if (canEditUser($user, 100) && userDj($user)) {
        $mysqli->query("UPDATE boom_users SET user_onair = 0 WHERE user_id = '{$user['user_id']}'");
        return 1;
    } else {
        return 0;
...
function makeDj($target)
{
    ...
    if (canEditUser($user, 100, 1) || boomAllow(100) && mySelf($user['user_id'])) {
...
function exGradyChoices($sel, $type, $min = 1)
{
    global $cody;
    $show_c = '';
    switch ($type) {
    ...
    for ($n = $min; $n <= 22; $n++) {
        ...
        $add_sel = '';
        if ($val == $sel) {
            $add_sel = '<i style="font-size: 11px;color: #fff;margin-top: 6px;" class="fa fa-check bccheck"></i>';
        }
        $show_c .= '<div style="min-height: 25px;" data="' . $val . '" class="color_switch ' . $c . ' ' . $back . '">' . $add_sel . '</div>';
    }
    return $show_c;
}
function exNameBg($sel, $type, $min = 1)
{
    global $cody;
    $show_c = '';
    switch ($type) {
...
function exListBg($sel, $type, $min = 1)
{
    global $cody;
    $show_c = '';
    switch ($type) {
...
function exListBgGlow($sel, $type, $min = 1)
{
    global $cody;
    $show_c = '';
    switch ($type) {
...
function movingGrad($sel, $type, $min = 1)
{
    global $cody;
    $show_c = '';
    switch ($type) {
...
function warnThisUser($id)
{
    ...
    if ($user['user_rank'] >= 95 || $user['user_role'] >= 4) {
        return 0;
    }
    if (boomAllow(95) || boomRole(4)) {
        $mysqli->query("UPDATE boom_users SET user_warn = user_warn + 1 WHERE user_id = '{$user['user_id']}'");
        boomNotify('warn_log', array('hunter' => $data['user_id'], 'target' => $user['user_id']));
        boomHistory('warn', array('target' => $user['user_id'], 'reason' => 'عدم الالتزام بقوانين الدردشة'));
        boomConsole('warn_log', array('target' => $user['user_id']));
        warnLog($user);
        return 1;
    } else {
        return 0;
...
function xbanAccount($id)
{
    ...
    systemBan($user, 'عدم الالتزام بالقوانين');
    $mysqli->query("INSERT INTO ex_dead_ban (hunter, target, ip, fprint, ban_time) VALUES ('{$data['user_id']}', '{$user['user_id']}', '{$user['user_ip']}', '{$user['ex_finger_print']}', '" . time() . "')");
    boomConsole('xban', array('target' => $user['user_id'], 'hunter' => $data['user_id'], 'reason' => 'عدم الالتزام بالقوانين'));
    boomHistory('xban', array('target' => $user['user_id'], 'reason' => 'عدم الالتزام بالقوانين'));
    return 1;
}
...
function blockRoom($id)
{
    ...
    if (canRoomAction($user, 5, 2) || boomAllow(100)) {
...
function muteRoom($id)
{
    ...
    if (canRoomAction($user, 4, 2) || boomAllow(100)) {
...
function removeRoomStaff($target)
{
    ...
    if (!betterRole($user['room_ranking']) && !boomAllow(100)) {
        return 0;
    }
...
function canRoomAction($user, $role, $type = 1)
{
    ...
    if (!betterRole($user['room_ranking'])) {
        return false;
    }
...
{
    if ($user['user_rank'] < 98 && !isBot($user) && !mySelf($user['user_id'])) {
        return true;
    }
}
...
{
    global $data;
    if ($data['user_role'] > $rank || boomAllow(100)) {
        return true;
    }
}
...
function canPutRoomRank($rank)
{
    global $data, $mysqli;
    $checkmod = $mysqli->query("SELECT * FROM boom_users WHERE user_id = '{$data['user_id']}'");
    if ($checkmod->num_rows > 0) {
        while ($fet = $checkmod->fetch_assoc()) {
            if ($rank == 4) {
                if ($fet['max_mod_room'] == 0) {
                    return false;
                }
            }
            if ($rank == 5) {
                if ($fet['max_staff_room'] == 0) {
                    return false;
                }
            }
        }
...
function isMusic($ext)
{
    $ext = strtolower($ext);
    $f = array(
        'audio/mpeg', 'audio/mp3', 'audio/x-mpeg', 'audio/x-mp3', 'audio/mpeg3',
...
function getCritera($c)
{
    switch ($c) {
        ...
        case 3:
            return "user_rank >= '20' AND user_rank < 30 AND user_bot = 0";
        case 4:
            return "user_rank = '95' AND user_bot = 0";
        case 5:
            return "user_rank = '97' AND user_bot = 0";
        case 6:
            return "user_bot > 0";
        case 7:
        ...
        case 10:
            return "user_rank = '98' AND user_bot = 0";
        case 11:
...
function tumbLinkingGif($img, $tumb)
{
    return '<a href="' . $img . '" class="fancybox"><img class="chat_image"src="' . $tumb . '"/></a> ';
}
function linking($content)
{
    if (!badWord($content)) {
        ...
        if (normalise($content, 1)) {
            ...
            $content = preg_replace('@https?:\/\/(www\.)?youtube.com/watch\?v=([\w_-]*)([' . $regex . ']*)?@ui', '<div style="display: inline;" class="chat_video_container"><div data="https://www.youtube.com/embed/$2" value="youtube" class="boom_youtube open_player hide_mobile"><i class="fab fa-youtube"></i> Youtube</div></div>', $content);
...
function customChatImg($source, $tumb = '')
{
    ...
    return '<a href="' . $source . '" class="fancybox"><i style="font-size:22px;" class="fa fa-image warn"></i></a>';
}
...
function musicProcessOrg($f, $r)
{
    $file = array(
        'file' => $f,
        'title' => $r
    );
    return boomTemplate('element/audio_music', $file);
}
...
function cleanListQuery($type)
{
    ...
    switch ($type) {
        ...
        case 'innactive_member':
            return "user_rank < 20 AND user_bot = 0 AND last_action <= '$innactive_delay' LIMIT 500";
        case 'account_delete':
            return "user_rank < 99 AND user_bot = 0 AND user_delete > 0 AND user_delete < '$delete_delay' LIMIT 500";
        default:
            return "";
    }
}
...
function canSendPrivate($priv)
{
    global $mysqli, $data;
    $user = userDetails($priv);
    $user2 = boomUserInfos($priv);
    ...
    if ($user['private_ask'] == 1) {
        if (!isOwner($data) && $user2['user_private'] == 0 || !isOwner($data) && $user2['ignored'] > 0) {
            $d['pt'] = 3;
        } else if ($ask_allowed == 1) {
            return true;
        } else if (isOwner($data) || $user2['user_private'] == 2 && haveFriendship($user2) || $user2['user_private'] == 1 || $user2['user_private'] == 3 && $data['user_rank'] >= 1) {
            return true;
        } else if (!isOwner($data) && $user2['user_private'] == 2 && !haveFriendship($user2) || !isOwner($data) && $user2['user_private'] == 3 && $data['user_rank'] < 1 || !isOwner($data) && $user_asked == 0) {
            return false;
        } else if ($user_asked == 1) {
            return false;
        }
    } else {
        if (isOwner($data) || $user2['user_private'] == 1 || $user2['user_private'] == 2 && haveFriendship($user2) || $user2['user_private'] == 3 && $data['user_rank'] >= 1) {
            return true;
        } else if (!isOwner($data) && $user2['user_private'] == 2 && !haveFriendship($user2) || !isOwner($data) && $user2['user_private'] == 3 && $data['user_rank'] < 1) {
            return false;
        } else if (!isOwner($data) && $user2['user_private'] == 0 || !isOwner($data) && $user2['ignored'] > 0) {
            return false;
        }
    }
    return true;
}
function accessProLimits($id, $col)
{
    global $mysqli, $data;
    ...
    if ($col == 0 && !isStaff($data['user_rank'])) {
        return false;
    }
    if ($col == 2 && !haveFriendship($user)) {
        return false;
    }
    if ($col == 3 && $data['user_rank'] < 1) {
        return false;
    }
    if ($user['ignored'] > 0) {
        return false;
...
function listAction($user)
{
    ...
    if (boomAllow(95) && !mySelf($id)) {
        $menu .= '<option value="kick">' . $lang['kick'] . '</option>';
    }
    if (boomAllow(95) && !mySelf($id)) {
        $menu .= '<option value="unkick">' . $lang['unkick'] . '</option>';
    }
    if (boomAllow(97) && !mySelf($id)) {
        $menu .= '<option value="ban">' . $lang['ban'] . '</option>';
    }
    if (boomAllow(98) && !mySelf($id)) {
        $menu .= '<option value="unban">' . $lang['unban'] . '</option>';
    }
    if (boomAllow(100) && !mySelf($id) && isStaff($user['user_rank']) || boomAllow(100) && !mySelf($id) && $user['user_role'] >= 4) {
        $menu .= '<option value="take_all_perm">سحب الصلاحيات</option>';
    }
    if (boomAllow(100) && !mySelf($id) && isStaff($user['user_rank']) || boomAllow(100) && !mySelf($id) && $user['user_role'] >= 4) {
        $menu .= '<option value="give_all_perm">منح الصلاحيات</option>';
    }
    return $menu;
}
function listRoomAction($user)
{
    ...
    if (boomAllow(100) || boomRole(6)) {
        ...
        $menu .= '<option value="room_block">' . $lang['block'] . '</option>';
        $menu .= '<option value="room_unblock">' . $lang['unblock'] . '</option>';
    }
    return $menu;
}
