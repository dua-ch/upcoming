<?php
if (!empty($boom['my_border'])) {
	$frame = '<img class="overlist ul_fr_bg" src="images/border/' . $boom['my_border'] . '"/>';
}
?>
<div id="boom_news<?php echo $boom['id']; ?>" data="<?php echo $boom['id']; ?>" class="news_box post_element">
	<div class="post_title">
		<div class="post_avatar_frame get_info" data="<?php echo $boom['user_id']; ?>">
			<img class="under ul_fr_av" src="<?php echo myAvatar($boom['user_tumb']); ?>"/>
			<?php echo $frame; ?>
		</div>
		...
		<div onclick="openDeletePost('news', <?php echo $boom['id']; ?>);" class="post_edit bcell_mid_center">
			<i class="fa fa-trash error"></i>
		</div>
		<?php } ?>
		<div onclick="showNewsReport();" class="post_edit bcell_mid_center">
