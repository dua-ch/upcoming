...
if (isset($_POST['set_room_name'], $_POST['allow_points_inroom'], $_POST['this_room_access'], $_POST['set_room_description'], $_POST['set_room_password'], $_POST['save_room'])) {
	if (!canEditRoom()) {
		die();
	}
	$player_check = 0;
	$name = escape($_POST['set_room_name']);
	$points = escape($_POST['allow_points_inroom']);
	$accesRoom = escape($_POST['this_room_access']);
	...
	if ($name == '' || strlen($name) > $cody['max_room_name']) {
		echo 4;
		die();
	}
	...
	$mysqli->query("UPDATE boom_rooms SET room_name = '$name', access = '$accesRoom', allow_points = '$points', description = '$description', password = '$password', room_player_id = '$player_id' WHERE room_id = '{$data['user_roomid']}'");
	echo 1;
	die();
}
...
if (isset($_POST['set_name']) && isset($_POST['set_pass']) && isset($_POST['set_type']) && isset($_POST['set_description'])) {
	...
	if ($max_room->num_rows >= $cody['max_room'] && !boomAllow(95)) {
		echo boomCode(5);
		die();
	}
...
if (isset($_POST['target'], $_POST['room_staff_rank'])) {
	...
	$mroom = roomInfo($data['user_roomid']);
	if (empty($target)) {
		echo 2;
		die();
	}
	if ($rank > 0 && boomRole(6) || $rank > 0 && boomAllow(100)) {
		if (canPutRoomRank($rank)) {
			if (checkMod($user['user_id'])) {
				...
				if(boomRole(6)){
					if ($rank == 4) {
						$mysqli->query("UPDATE boom_users SET max_mod_room = max_mod_room - 1 WHERE user_id = '{$data['user_id']}'");
					}
					if ($rank == 5) {
						$mysqli->query("UPDATE boom_users SET max_staff_room = max_staff_room - 1 WHERE user_id = '{$data['user_id']}'");
					}
				}
			} else {
				$mysqli->query("UPDATE boom_room_staff SET room_rank = '$rank' WHERE room_id = '{$data['user_roomid']}' AND room_staff = '{$user['user_id']}'");
			}
			boomNotify('change_room_rank', array('hunter' => $data['system_id'], 'target' => $user['user_id'],'rank' => $rank, 'custom' => $mroom['room_name']));
		} else {
			die();
		}
		...
		$mysqli->query("UPDATE boom_users SET user_role = 0, user_color = 'user' WHERE user_id = '{$user['user_id']}' AND user_roomid = '{$data['user_roomid']}'");
	}
	boomConsole('change_room_rank', array('target' => $user['user_id'], 'rank' => $rank));
	echo 1;
	die();
}
if (isset($_POST['admin_add_room'], $_POST['admin_set_name'], $_POST['admin_set_pass'], $_POST['admin_set_type'], $_POST['admin_set_description']) && boomAllow(98) && canRoom()) {
