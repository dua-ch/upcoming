...
$data_list = $mysqli->query("
	...
	user_id, user_mute, user_regmute, room_mute, last_action, user_bot, user_role, user_mood, country, private_ask, my_border, ex_name_bg, ex_name_bg_glow, ex_av_border
	FROM `boom_users` 
	WHERE `user_roomid` = {$data["user_roomid"]}  AND last_action > '$check_action' AND user_status != 6 || user_bot = 1 ORDER BY `user_rank` DESC, `user_role` DESC, `user_name` ASC 
");

if ($data['max_offcount'] > 0) {
	$offline_list = $mysqli->query("
		SELECT user_name, user_mobile, user_color, user_font, user_rank, user_dj, user_onair, user_join, user_tumb, user_status, user_sex, user_age, user_cover, country,
		user_id, user_mute, user_regmute, room_mute, last_action, user_bot, user_role, user_mood, country, private_ask, my_border, ex_av_border
		FROM `boom_users`
...
if ($data_list->num_rows > 0) {
	while ($list = $data_list->fetch_assoc()) {
		if ($list['user_dj'] == 1 && $list['user_onair'] == 1) {
			...
			if($list['user_rank'] >= 88){
				$chat_staff .= createUserlist($list);
			} elseif($list['user_role'] == 6){
				$room_staff1 .= createUserlist($list);
			} elseif($list['user_role'] == 5){
				$room_staff2 .= createUserlist($list);
			} elseif($list['user_role'] == 4){
				$room_staff3 .= createUserlist($list);
			} else {
...
<div id="container_user">
	...
	<div class="user_count">
		<div class="bcell">
			<?php echo $lang['online']; ?> <span class="ucount ok_btn"><i class="fa fa-users"></i> <?php echo $online_count; ?></span>
		</div>
	</div>
	<div class="online_user"><?php echo $chat_staff . $room_staff1 . $room_staff2 . $room_staff3 . $online_user; ?></div>
	<?php if ($offline_user != '') { ?>
		<div class="user_count">
			<div class="bcell">
