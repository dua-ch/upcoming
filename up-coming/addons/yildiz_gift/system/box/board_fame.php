...
function quizLeader($lead, $rank){
	...
	return '<div class="ulist_item ' . $add_me . '">
				...
				<div class="score_lm">
					<i class="fa fa-fire" style="color: red;"></i> ' . $lead['user_fame'] . '
				</div>
			</div>';
}

$get_leader = $mysqli->query("SELECT * FROM boom_users WHERE user_fame > 0 AND user_bot = 0 ORDER BY user_fame DESC LIMIT 100");
