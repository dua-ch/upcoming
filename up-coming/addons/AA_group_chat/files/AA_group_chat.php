	...
	<script data-cfasync="false" type="text/javascript">
		...
		sendUserInvite = function(source) {
			...
			$.ajax({
				...
				success: function(response) {
					if (response == 1) {
						callSaved('تم ارسال الدعوة', 1);
						$('#start_gchat').show();
						$(source).html('');
						$(source).html('<i id="invite_gchat_ico" class="fa fa-check success"></i>');
					} else if (response == 3) {
						callSaved('لا يمكنك ارسال دعوة الى نفسك', 3);
						$(source).html('<i id="invite_gchat_ico" class="fa fa-times error"></i>');
					} else if (response == 4) {
						callSaved('العضو في محادثه جماعية بالفعل', 3);
						$(source).html('<i id="invite_gchat_ico" class="fa fa-times error"></i>');
					} else {
						callSaved('لقد تم ارسال دعوة لهذا العضو بالفعل', 3);
						$(source).html('<i id="invite_gchat_ico" class="fa fa-times error"></i>');
					}
				},
			});
		...
		$(document).ready(function() {
			...
			$('#group_modal_content').on('click', '#show_gchat .username', function() {
				emoticon('group_content', $(this).text());
			});
			
			appAvMenu('other', 'share warn', 'دعوة محادثة جماعية', 'sendUserInvite(this);startMyGroup();');
			appAvMenu('staff', 'share warn', 'دعوة محادثة جماعية', 'sendUserInvite(this);startMyGroup();');
			appAvMenu('roomstaff', 'share warn', 'دعوة محادثة جماعية', 'sendUserInvite(this);startMyGroup();');
			...
			<?php if ($data['user_group'] == '') { ?>
				appLeftMenu('comments error', 'محادثة جماعية', 'openGroupChat();');
			<?php } ?>
	...
	<div id="group_modal" class="privelem privhide">
		<div class="gchat_top top_panel btable top_background">
			<div id="private_name" class="bcell_mid bellips">
				<i class="fa fa-comments"></i> محادثة جماعية
			</div>
