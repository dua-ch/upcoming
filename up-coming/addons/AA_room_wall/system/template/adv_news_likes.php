...
SELECT ex_news_like.*, boom_users.*  
FROM ex_news_like, boom_users 
WHERE ex_news_like.like_post = '$id' AND boom_users.user_id = ex_news_like.uid
");
...
if($get_like->num_rows > 0){
	...
	while($likes = $get_like->fetch_assoc()){
		switch($likes['like_type']){
			case 1:
				$like .= exTemplate('system/template/user', $likes);
				$like_count++;
				break;
			case 2:
				$dislike .= exTemplate('system/template/user_lazy', $likes);
				$dislike_count++;
				break;
			case 3:
				$love .= exTemplate('system/template/user_lazy', $likes);
				$love_count++;
				break;
			case 4:
				$funny .= exTemplate('system/template/user_lazy', $likes);
				$funny_count++;
				break;
		}
	}
